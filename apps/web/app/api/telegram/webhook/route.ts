...
export const POST = withMiddleware(async (request) => {
  ...
  if (isGroupChat(chatType)) {
    ...
    console.log(
      `✅ Group rate limit OK: ${rateLimit.count}/${rateLimit.remaining + rateLimit.count}`
    )
  ...
  const agentResponse = await processAgentMessage({
    ...
    source: 'telegram', // Enable Telegram-specific templates (raids, memes, etc.)
  })
  ...
  switch (cmd) {
    ...
    case 'help':
      await bot.telegram.sendMessage(
        ...
        `🆘 *Caisper Help Guide*\n\n` +
          ...
          `/mute - Mute bot auto-responses\n` +
          `/unmute - Unmute bot auto-responses\n\n` +
          `*Ask me anything!*\n` +
    ...
    case 'mute':
      ...
      try {
        ...
        if (!isAdmin) {
          await bot.telegram.sendMessage(chatId, '⚠️ Only group admins can mute the bot')
          break
        }
    ...
    case 'unmute':
      ...
      try {
        ...
        if (!isAdmin) {
          await bot.telegram.sendMessage(chatId, '⚠️ Only group admins can unmute the bot')
          break
        }
