...
export const POST = withMiddleware(async (request) => {
  ...
  const paymentRequired = await requireX402Payment(request, { priceUsdc: 0.01 }, getConvexClient())
    ...
    agents: [
      {
        ...
        discoverySource: 'account_scan', // API registration treated as account scan
        slot: 0,
      },
    ],
  })
  ...
  if (name || description || x402Enabled !== undefined) {
    try {
      ...
      console.warn(
        `[Agent Register] Metadata update without signature verification for ${agentAddress}`
      )
