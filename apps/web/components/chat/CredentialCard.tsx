  ...
  return (
    <div className="w-full max-w-md my-2 rounded-xl border border-white/10 bg-black/40 backdrop-blur-md overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-white/10 bg-white/5 flex items-center justify-between">
        ...
        <div
          className={`text-xs px-2 py-0.5 rounded border ${validCount === totalCount ? 'bg-lime-500/10 border-lime-500/30 text-lime-400' : 'bg-amber-500/10 border-amber-500/30 text-amber-400'}`}
        >
            ...
            <div key={idx} className="group">
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-8 h-8 rounded flex items-center justify-center border ${cred.isValid ? 'bg-lime-500/10 border-lime-500/20' : 'bg-red-500/10 border-red-500/20'}`}
                  >
                    {cred.isValid ? (
                      <CheckCircle2 className={`w-4 h-4 ${statusColor}`} />
                    ) : (
                      <XCircle className={`w-4 h-4 ${statusColor}`} />
                    )}
                  </div>
                  <div>
                ...
                <FileJson
                  className={`w-4 h-4 text-white/20 transition-transform ${isExpanded ? 'text-lime-400' : 'group-hover:text-white/40'}`}
                />
              </button>
                    ...
                    <div className="px-4 py-3 space-y-2 text-xs border-t border-white/5 shadow-inner">
                      <div className="flex justify-between">
                        ...
                        <span className={statusColor}>
                          {cred.isValid ? 'Active' : 'Expired / Revoked'}
                        </span>
                      </div>
                      {cred.issuedAt && (
                        <div className="flex justify-between">
