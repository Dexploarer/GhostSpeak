          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm font-bold ${statusColor}`}>
                {status} {isPaymentRequired ? 'Payment Required' : isSuccess ? 'OK' : 'Error'}
              </span>
            </div>
              ...
              <div className="mt-2 text-xs space-y-1 bg-black/20 p-2 rounded">
                {data.payment.amount && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-amber-200">{data.payment.amount}</span>
                  </div>
                )}
                {data.payment.token && (
                  <div className="flex justify-between">
                    <span className="text-white/40">Token:</span>{' '}
                    <span className="text-white/60">{data.payment.token}</span>
                  </div>
                )}
                {data.payment.address && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-white/60 font-mono">
                      {data.payment.address.slice(0, 6)}...{data.payment.address.slice(-4)}
                    </span>
                  </div>
                )}
              </div>
