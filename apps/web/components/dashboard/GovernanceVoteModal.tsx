...
export function GovernanceVoteModal({ voterAddress, targetAgentAddress, agentName }: GovernanceVoteModalProps) {
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="bg-[#111111] border-white/10 text-white sm:max-w-md">
                ...
                <div className="space-y-6 pt-4">
                    {/* Power Status */}
                    <div className={cn("p-3 rounded-lg border flex items-start gap-3", hasPower ? "bg-blue-500/10 border-blue-500/20" : "bg-red-500/10 border-red-500/20")}>
                        <Medal className={cn("w-5 h-5 mt-0.5", hasPower ? "text-blue-400" : "text-red-400")} />
                        <div>
                            <h4 className={cn("text-sm font-medium", hasPower ? "text-blue-400" : "text-red-400")}>
                                {hasPower ? "You have Voting Rights" : "No Voting Power"}
                            </h4>
                            <p className="text-xs text-white/60 mt-1">
                                Your Power: <strong>{powerData ? powerData.power : '...'}</strong> (Based on Transaction Volume & Count)
                            </p>
                        </div>
                    </div>
