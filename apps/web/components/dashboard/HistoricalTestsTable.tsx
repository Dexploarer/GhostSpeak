...
export function HistoricalTestsTable() {
    ...
    if (logs.length === 0) return <div className="p-4 text-white/40 text-sm">No historical tests found.</div>
                    ...
                    <tbody className="divide-y divide-white/5">
                        {logs.map((log: any) => (
                            <tr key={log._id} className="hover:bg-white/5 transition-colors">
                                ...
                                <td className="px-4 py-3 max-w-xs truncate text-white/60" title={JSON.stringify(log.details)}>
