        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] w-full max-w-4xl mx-auto p-4">
            ...
            <div className="mb-8 w-full max-w-lg z-20">
                <div className="flex space-x-2">
                    ...
                    >
                        {isSummoning ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Search className="w-4 h-4 mr-2" />}
                        SUMMON
                    </Button>
                </div>
            </div>
            ...
            >
                ...
                <div className="absolute top-[55%] w-full flex justify-center space-x-8 text-2xl font-bold opacity-60">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9, 0].map(n => <span key={n}>{n}</span>)}
                </div>

                {/* GOODBYE */}
                <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-3xl font-bold opacity-80">GOODBYE</div>
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm text-left border-t border-purple-900/50 pt-4">
                            <div>
                                ...
                                <span className={cn(
                                    "text-lg",
                                    report.summary.reliability === "Trustworthy" ? "text-green-400" :
                                        report.summary.reliability === "Deceptive" ? "text-red-400" : "text-yellow-400"
                                )}>{report.summary.reliability}</span>
                            </div>
                            <div>
                                <span className="text-gray-500 block">Ghost Score</span>
                                <span className="text-lg text-purple-200">{report.reputation?.ghostScore || "N/A"}</span>
                            </div>
                        </div>
                    </motion.div>
                )}
    ...
    return (
        ...
        >
            ...
            <svg viewBox="0 0 100 130" className="w-full h-full drop-shadow-2xl opacity-90">
                <path d="M50 0 C 80 0, 100 30, 100 60 C 100 100, 50 130, 50 130 C 50 130, 0 100, 0 60 C 0 30, 20 0, 50 0" fill="#fefbd8" stroke="#4a3b2a" strokeWidth="2" />
