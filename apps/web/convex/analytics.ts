...
export const getVerificationVolume = query({
    ...
    handler: async (ctx, args) => {
        ...
        const data = Array.from(dailyMap.entries()).map(([name, { count, scores }]) => ({
            name,
            count,
            quality: scores.length > 0 ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length / 100) : 0,
        }))
...
export const getAnalyticsSummary = query({
    ...
    handler: async (ctx, args) => {
        ...
        let currentUsage: Array<{ statusCode: number; responseTime: number }> = []
        let previousUsage: Array<{ statusCode: number; responseTime: number }> = []
        ...
        for (const key of apiKeys) {
            ...
            currentUsage.push(...current.map((u) => ({
                statusCode: u.statusCode,
                responseTime: u.responseTime,
            })))
            previousUsage.push(...previous.map((u) => ({
                statusCode: u.statusCode,
                responseTime: u.responseTime,
            })))
        }
        ...
        return {
            ...
            totalRequestsTrend: Math.round(calcTrend(currentUsage.length, previousUsage.length) * 10) / 10,
...
export const getDetailedMetrics = query({
    ...
    handler: async (ctx, args) => {
        ...
        const avgResponseSize = responseCount > 0
            ? `${(totalResponseSize / responseCount).toFixed(1)} KB`
            : '0 KB'
