...
export const checkAndUpdateTier = action({
    ...
    handler: async (ctx, { walletAddress }): Promise<{
        tier: string;
        ...
        try {
            ...
            try {
                const priceResponse = await fetch(
                    `https://api.jup.ag/price/v2?ids=${GHOST_TOKEN_MINT}`,
            ...
            console.log(`[MessageTier] ${walletAddress.slice(0, 8)}: ${ghostBalance} $GHOST = $${ghostBalanceUsd.toFixed(2)}`)
