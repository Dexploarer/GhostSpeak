...
export const castVote = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!hasPower) {
            ...
            const agentStats = await ctx.db.query('agentReputationCache').withIndex('by_address', q => q.eq('agentAddress', args.voterAddress)).first()
            if (agentStats) hasPower = true
        }

        if (!hasPower) {
            throw new Error("No voting power. Participate in x402 transactions to earn the right to vote.")
        }
