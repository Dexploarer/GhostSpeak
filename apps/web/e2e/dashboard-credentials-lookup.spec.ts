...
async function installMockWalletStandard(
  ...
  // Use a more robust approach: inject mock after page load using evaluate
  // This ensures the mock is applied after the page scripts have loaded
  await page.addInitScript(
    ({ walletAddress, includeSignMessage }) => {
      const walletName = 'E2E Test Wallet'

      // Set up localStorage BEFORE any wallet provider initialization
      try {
      ...
      const registerMethods = [
        ...
        () => {
          try {
            // For wallets that check the window object directly
            if ((window as any).WalletStandard) {
              ;(window as any).WalletStandard.register(wallet)
            }
          } catch {}
        },
      ]
  ...
  // After page load, try to inject mock via evaluate if needed
  await page.evaluate(
    ({ walletAddress, includeSignMessage }) => {
      const walletName = 'E2E Test Wallet'
...
async function ensureWalletAutoConnected(page: Page, { walletAddress }: { walletAddress: string }) {
  ...
  // Try to find wallet button with multiple possible states
  ...
  // Wait for any wallet-related button to be visible
  await expect(
    walletButton,
...
async function gotoDashboardViaPortal(page: Page) {
  ...
  try {
    ...
    // If not visible, check if we're already on dashboard
    const currentUrl = page.url()
    if (/\/dashboard/.test(currentUrl)) {
      // Already on dashboard, just verify heading
      await expect(
...
test.describe('Dashboard /credentials - VC lookup UX', () => {
  test('agent address input → agent card → VC list → VC details panel', async ({ page }) => {
    ...
    await expect(page.getByTestId('credentials-lookup-section')).toBeVisible({ timeout: 30000 })
    ...
    ).toContainText('Proof: not available in web yet', { timeout: 30000 })
    ...
    ).toContainText('did:', { timeout: 30000 })
    ...
    if (credentialId) {
      ...
      ).toHaveText(credentialId, { timeout: 30000 })
    }
  })
})
