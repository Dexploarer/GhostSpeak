...
export const getUserPortfolioAction: Action = {
    ...
    description: "Get the authenticated user's portfolio, reputation scores (Ecto/Ghosthunter), and recent activity",
    ...
    handler: async (
        ...
        try {
            ...
            if ((reputation.ghosthunter?.score || 0) > 7500 || (reputation.ecto?.score || 0) > 7500) {
                myTake = "You're a power user! Your presence in the GhostSpeak network is felt across the blockchain. Respect. 🫡"
            } else if (stats.totalVerifications > 10) {
                myTake = "You've been busy verifying spirits! Keep it up—the network depends on hunters like you. 🕵️‍♂️"
            } else {
                myTake = "You're just getting started, but the foundations are solid. Want me to find some unclaimed agents for you to register? 👻"
            }
