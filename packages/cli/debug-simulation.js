...
async function debugSimulation() {
  console.log('🔍 Debug Transaction Simulation');
  ...
  try {
    ...
    const agentId = 'simple-test-123';
    const agentType = 0;
    const metadataUri = 'https://example.com/metadata.json'; // Simple URL
    ...
    try {
      const simulationResult = await rpc.simulateTransaction(signedTransaction, {
        commitment: 'confirmed',
        accounts: {
          encoding: 'base64',
          addresses: [agentAccount, userRegistry]
        }
      }).send();
      ...
      if (simulationResult.value.accounts) {
        ...
        simulationResult.value.accounts.forEach((account, i) => {
          if (account) {
            console.log(`  ${i + 1}. Owner: ${account.owner}, Lamports: ${account.lamports}, Data: ${account.data.length} bytes`);
          } else {
