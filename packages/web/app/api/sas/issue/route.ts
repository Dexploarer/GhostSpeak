...
async function getSASConfig() {
	...
	const authorizedSignerBytes = Uint8Array.from(
		Buffer.from(authorizedSignerKeypair, 'base64')
	)
...
export async function POST(request: NextRequest) {
	try {
		...
		const { value: latestBlockhash } = await config.client.rpc
			.getLatestBlockhash()
			.send()
