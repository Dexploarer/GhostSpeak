...
export function LiveObservationFeed() {
  ...
  const handleVote = async (e: React.MouseEvent, observationId: any, voteType: 'upvote' | 'downvote') => {
    e.stopPropagation()
    ...
    <div className="space-y-4 max-w-4xl mx-auto">
      {observations.map((obs) => {
        ...
        return (
          ...
          >
            <div className={`bg-[#111111] border rounded-xl overflow-hidden transition-all ${
              isExpanded ? 'border-primary/50 ring-1 ring-primary/20' : 'border-white/10 hover:border-white/20'
            }`}>
              <CollapsibleTrigger className="w-full group">
                <div className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white font-medium truncate max-w-[300px]" title={obs.endpoint?.endpoint}>
                  ...
                  <div className="flex items-center gap-3">
                    {/* Voting Buttons */}
                    <div className="flex items-center bg-white/5 rounded-lg border border-white/5 mr-2" onClick={(e) => e.stopPropagation()}>
                      <button
                        ...
                        className={`p-1.5 px-2 flex items-center gap-1.5 text-xs transition-colors hover:bg-white/10 ${
                          myVote === 'upvote' ? 'text-green-400 bg-green-500/10' : 'text-white/40 hover:text-white/80'
                        }`}
                      ...
                      <button
                        ...
                        disabled={!walletAddress}
                        className={`p-1.5 px-2 flex items-center gap-1.5 text-xs transition-colors hover:bg-white/10 ${
                          myVote === 'downvote' ? 'text-red-400 bg-red-500/10' : 'text-white/40 hover:text-white/80'
                        }`}
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-mono ${
                      obs.responseStatus === 200 ? 'bg-green-500/10 text-green-400' :
                      obs.responseStatus === 402 ? 'bg-orange-500/10 text-orange-400' :
                          ...
                          <div key={i} className={`flex gap-3 ${
                            ...
                            <div className={`max-w-[85%] rounded-lg p-3 ${
                              ...
                              <div className="flex items-center justify-between gap-4 mb-1 opacity-50 text-[10px] uppercase">
                                <span>
                                  {entry.role === 'user' ? 'Caisper (Probe)' : 
                                   entry.role === 'system' ? 'System Event' : 'Agent Response'}
                                </span>
                  ...
                  {obs.caisperNotes && (
                    <div className="mb-4 p-3 rounded bg-white/5 border border-white/5 text-sm">
                      <div className="flex items-center gap-2 text-white/60 mb-1">
                        ...
                        <span className="uppercase text-[10px] tracking-wider">Caisper Analysis</span>
                      </div>
