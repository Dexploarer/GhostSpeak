...
export const listEndpoints = query({
  ...
  handler: async (ctx, args) => {
    let q = ctx.db.query('observedEndpoints')

    if (args.agentAddress) {
      q = q.withIndex('by_agent', (q) => q.eq('agentAddress', args.agentAddress!))
    ...
    const endpoints = await q.collect()
    return args.limit ? endpoints.slice(0, args.limit) : endpoints
  },
})
...
export const recordTestResult = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (endpoint) {
      ...
      const avgResponseTimeMs = Math.round((prevAvgTime * (totalTests - 1) + args.responseTimeMs) / totalTests)
      const avgQualityScore = Math.round((prevAvgQuality * (totalTests - 1) + args.qualityScore) / totalTests)
...
export const compileDailyReport = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const tests = await ctx.db
      .query('endpointTests')
      .withIndex('by_agent', (q) => q.eq('agentAddress', args.agentAddress))
      .filter((q) => q.and(q.gte(q.field('testedAt'), startOfDay), q.lt(q.field('testedAt'), endOfDay)))
      .collect()
    ...
    const avgResponseTimeMs = Math.round(tests.reduce((sum, t) => sum + t.responseTimeMs, 0) / tests.length)
    ...
    const trustworthiness = Math.round((successRate * 40 + verificationRate * 40 + (avgQualityScore / 100) * 20))
    ...
    const existing = await ctx.db
      .query('dailyObservationReports')
      .withIndex('by_agent_date', (q) => q.eq('agentAddress', args.agentAddress).eq('date', args.date))
      .first()
...
export const runHourlyTests = internalAction({
  ...
  handler: async (ctx) => {
    ...
    console.log(`[Observation] Hourly run complete: ${testsRun} tests, $${totalSpent.toFixed(4)} spent`)
  ...
  handler: async (ctx) => {
    ...
    for (const agentAddress of uniqueAgents) {
      try {
        ...
        console.error(`[Observation] Failed to compile report for ${agentAddress}: ${error.message}`)
      }
    }
 ...
 */
export const getTestsInRange = internalMutation({
  args: {
    startTime: v.number(),
    endTime: v.number(),
  },
  handler: async (ctx, args) => {
    return await ctx.db
      .query('endpointTests')
      .filter((q) => q.and(q.gte(q.field('testedAt'), args.startTime), q.lt(q.field('testedAt'), args.endTime)))
      .collect()
  },
})
