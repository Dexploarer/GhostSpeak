/**
 * Transaction Failed Recording Retry Mechanism
 *
 ...
 * with on-chain polling (Phase 2 of transaction integration improvements).
 *
 * **Migration path:**
...
export const storeFailedRecording = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      console.log('[Transaction Retries] Payment already queued for retry:', args.paymentSignature)
      return existing._id
    }
    ...
    console.log('[Transaction Retries] Stored failed recording:', {
      id: recordingId,
      paymentSignature: args.paymentSignature,
...
export const markRecordingSucceeded = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!recording) {
      console.warn('[Transaction Retries] Recording not found:', args.paymentSignature)
      return null
    }
    ...
    console.log('[Transaction Retries] Marked recording as succeeded:', {
      paymentSignature: args.paymentSignature,
...
export const retryFailedRecording = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!recording) {
      console.error('[Transaction Retries] Recording not found:', args.recordingId)
      return { success: false, error: 'Recording not found' }
    }

    // Check retry limits
    if (recording.retryCount >= recording.maxRetries) {
      console.warn('[Transaction Retries] Max retries reached:', {
        id: args.recordingId,
    ...
    console.log('[Transaction Retries] Attempting retry:', {
      id: args.recordingId,
