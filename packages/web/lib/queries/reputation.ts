...
async function calculateComprehensiveReputation(
  ...
  const marketplaceActivity = await getAgentMarketplaceActivity(agentAddress)
...
async function getAgentJobHistory(agentAddress: string) {
  ...
  try {
    ...
    const agentWorkOrders = allWorkOrders.filter((wo: any) => 
      wo.data?.agent?.toString() === agentAddress
    )
    
    return agentWorkOrders.map((wo: any) => ({
      id: wo.address,
      type: 'work_order',
...
async function getAgentEscrowHistory(agentAddress: string) {
  ...
  try {
    ...
    const agentEscrows = allEscrows.filter((escrow: any) =>
      escrow.data?.agent?.toString() === agentAddress
    )
    
    return agentEscrows.map((escrow: any) => ({
      id: escrow.address,
      ...
      completedAt: escrow.data?.completedAt ? new Date(Number(escrow.data.completedAt) * 1000) : null,
...
async function getAgentMarketplaceActivity(agentAddress: string) {
  ...
  try {
    ...
    const agentListings = allListings.filter((listing: any) =>
      listing.data?.provider?.toString() === agentAddress
    )
    
    return {
      totalListings: agentListings.length,
      activeListings: agentListings.filter((l: any) => l.data?.isActive).length,
      totalSales: agentListings.reduce((sum: number, l: any) => sum + (l.data?.purchaseCount || 0), 0),
    }
  } catch (error) {
 ...
 */
function calculateCategoryScores(jobHistory: any[]): Record<string, number> {
  ...
  for (const [category, stats] of Object.entries(categoryStats)) {
    categoryScores[category] = stats.total > 0 
 ...
 */
function calculateRiskScore(agentAccount: any, jobHistory: any[], escrowHistory: any[]): number {
  let riskScore = 0
 ...
 */
function determineTrustLevel(score: number, riskScore: number, totalJobs: number): ReputationMetrics['trustLevel'] {
 ...
 */
function calculateEarnedBadges(agentAccount: any, jobHistory: any[], escrowHistory: any[]) {
 ...
 */
function generatePerformanceHistory(jobHistory: any[], escrowHistory: any[]) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return filteredAgents
        .sort((a: any, b: any) => (b.data?.reputationScore || 0) - (a.data?.reputationScore || 0))
        .slice(0, limit)
        .map((agent: any) => ({
          ...
          tier: calculateReputationTier(agent.data?.reputationScore || 0, agent.data?.totalJobsCompleted || 0),
        }))
    },
