...
import { serializeAgentIdentityData, deserializeAgentIdentityData } from './borsh-serializer'
...
export async function issueAttestation(params: {
  ...
  const { client, payer, authority, authorizedSigner, schemaType, data, nonce, expiryDays } = params

  // Validate credential data
  if (!validateCredentialData(schemaType, data)) {
    throw new Error(`Invalid credential data for schema type: ${schemaType}`)
  }
  ...
  const expiryTimestamp =
    Math.floor(Date.now() / 1000) + (expiryDays || defaultExpiry) * 24 * 60 * 60
...
export async function revokeAttestation(params: {
  ...
  const { client, payer, authority, authorizedSigner, schemaType, nonce } = params
...
> {
  ...
  const schemaTypes: Array<
    'AGENT_IDENTITY' | 'REPUTATION_TIER' | 'PAYMENT_MILESTONE' | 'VERIFIED_STAKER' | 'VERIFIED_HIRE'
  > = ['AGENT_IDENTITY', 'REPUTATION_TIER', 'PAYMENT_MILESTONE', 'VERIFIED_STAKER', 'VERIFIED_HIRE']
