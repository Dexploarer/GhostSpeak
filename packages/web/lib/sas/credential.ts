 ...
 */
export async function getGhostSpeakCredentialPda(authority: Address): Promise<Address> {
 ...
 */
export async function getGhostSpeakCredential(client: SolanaClient, authority: Address) {
...
export async function isAuthorizedSigner(
  ...
  try {
    ...
    return credential.data.authorizedSigners.some((s) => s.toString() === signer.toString())
  } catch (error) {
