...
pub fn require_verified_agent(agent: &Account<Agent>) -> Result<()> {
    require!(
        agent.is_verified,
        GhostSpeakError::UnverifiedAgent
    );
...
pub fn validate_agent_supports_token(
    agent: &Account<Agent>,
    token_mint: &Pubkey,
) -> Result<()> {
    require!(
...
pub fn require_a2a_support(agent: &Account<Agent>) -> Result<()> {
    require!(
        agent.supports_a2a,
        GhostSpeakError::A2ANotSupported
    );
...
pub fn validate_agent_reputation(
    agent: &Account<Agent>,
    min_reputation: u32,
) -> Result<()> {
    require!(
