...
impl RateLimiter {
    ...
    pub fn initialize(&mut self, authority: Pubkey, config: RateLimitConfig, bump: u8) -> Result<()> {
    ...
    pub fn get_operation_limit(&self, operation: &str) -> (u16, i64) {
        if let Some(op_limit) = self.operation_limits
            .iter()
            ...
            (self.global_config.default_limit, self.global_config.window_duration)
        }
    }
}
...
impl UserRateLimit {
    ...
    pub fn check_rate_limit(
        ...
        let op_limit = rate_limiter.operation_limits
            .iter()
        ...
        if let Some(op) = op_limit {
            if op.allow_burst {
                ...
                let recent_requests = self.request_timestamps
                    .iter()
