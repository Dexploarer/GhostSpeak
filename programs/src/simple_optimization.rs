...
macro_rules! optimize_compute_budget {
    ($instruction:expr, $accounts:expr) => {{
        ...
        msg!(
            "Optimized compute budget for {}: {} CU",
            $instruction,
            base_budget
        );
        base_budget
...
impl ValidationHelper {
    ...
    pub fn validate_string_length(input: &str, max_length: usize, field_name: &str) -> Result<()> {
        if input.len() > max_length {
            msg!(
                "Input validation failed: {} exceeds maximum length of {}",
                field_name,
                max_length
            );
    ...
    pub fn validate_range(value: u64, min: u64, max: u64, field_name: &str) -> Result<()> {
        if value < min {
            msg!(
                "Validation failed: {} is below minimum of {}",
                field_name,
                min
            );
        ...
        if value > max {
            msg!(
                "Validation failed: {} exceeds maximum of {}",
                field_name,
                max
            );
...
impl InputValidator {
    ...
    pub fn validate_payment_amount(amount: u64, field_name: &str) -> Result<()> {
        ...
        use crate::{MAX_PAYMENT_AMOUNT, MIN_PAYMENT_AMOUNT};

        if amount < MIN_PAYMENT_AMOUNT {
            msg!(
                "Payment amount {} for {} is below minimum of {}",
                amount,
                field_name,
                MIN_PAYMENT_AMOUNT
            );
        ...
        if amount > MAX_PAYMENT_AMOUNT {
            msg!(
                "Payment amount {} for {} exceeds maximum of {}",
                amount,
                field_name,
                MAX_PAYMENT_AMOUNT
            );
...
impl FormalVerification {
    pub fn verify_work_order_transition(
        _old_status: u8,
        _new_status: u8,
        _context: &str,
    ) -> Result<()> {
