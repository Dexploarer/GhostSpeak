...
use crate::{GhostSpeakError, MAX_NAME_LENGTH, MAX_URL_LENGTH, 
            MAX_DESCRIPTION_LENGTH, MAX_CAPABILITIES_COUNT,
...
pub fn validate_string_input(
    ...
    if !allow_special_chars {
        ...
        if input.contains('<') || input.contains('>') || input.contains("javascript:") 
           || input.contains("data:") || input.contains("..") {
...
pub fn validate_agent_inputs(
    ...
    validate_string_input(description, "description", MAX_DESCRIPTION_LENGTH, false, true)?;
...
pub fn validate_capabilities(capabilities: &[String]) -> Result<()> {
    ...
    for capability in capabilities {
        ...
        require!(
            capability.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-' || c == ' '),
            GhostSpeakError::InvalidInputFormat
        );
    }
...
pub fn safe_arithmetic(a: u64, b: u64, operation: &str) -> Result<u64> {
    match operation {
        "add" => {
            a.checked_add(b).ok_or(error!(GhostSpeakError::ArithmeticOverflow))
        },
        "sub" => {
            a.checked_sub(b).ok_or(error!(GhostSpeakError::ArithmeticUnderflow))
        },
        "mul" => {
            a.checked_mul(b).ok_or(error!(GhostSpeakError::ArithmeticOverflow))
        },
        "div" => {
            if b == 0 {
